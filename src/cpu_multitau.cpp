...
MultiTauAutocorrelator:: MultiTauAutocorrelator(float * data, unsigned row, unsigned col):
    nrows_(row), ncols_(col), ntime_(col), idx_(0), len_(0) {
    if (ntime_ % 2) ntime_--;

    signal_ = new float[nrows_ * ncols_];
    std::memcpy(signal_, data, sizeof(float) * nrows_ * ncols_);
    g2s_ = nullptr;
    tau_ = nullptr;
}
...
void MultiTauAutocorrelator::run_level(unsigned ntaus) {
    ...
    for (unsigned j = 0; j < nrows_; j++) {
        for (unsigned tau = 0; tau < ntaus; tau++) {
            ...
            unsigned row_id = j * ncols_;
            for (unsigned i = 0; i < ntime_ - tau; i++) {
                unsigned l = row_id + i;
            ...
            g2s_[idx_ + j * len_ +  tau] = (t1 * (ntime_ -tau)) / (t2 * t3 );
        }
    }
}

void MultiTauAutocorrelator::process(unsigned tpl, float dt) {
    if (tpl % 2) tpl--;
    unsigned levels = (unsigned) std::log2(ntime_/tpl) + 1; 
    len_ = (levels + 1 ) * (tpl / 2);
    tau_ = new float[len_];
    g2s_ = new float[nrows_ * len_];
